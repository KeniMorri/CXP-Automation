...
function addMealItems(package) {
	...
	console.log(package);
	//High Level Check
	if(package[0] == '-1' && highLevelCodes.split('x')[0] == 'Y') {
		console.log('Alright, highlvl is fucked up, probably didnt load fully');
	}
	else {
		nightmare
		.type('input#numToAdd', '')
		...
		.evaluate(function(package) {
			var row = 0;
			var count = 0;
			var count = document.getElementById('lastRowNumber').value;
			/*
