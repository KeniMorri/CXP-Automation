...
function init() {
	Delay.init();
	scafoldNightmare();
	Log.initBasic('../Archive/ADVMatLogs/')
...
function SPMLPassLoader() {
	...
	var buffer = fs.readFileSync('./AdvancedMaterialUpdater/advMaterialInput.json');
		...
		.evaluate(function(package) {
				...
				if(document.getElementById('commentTexts_0').value.search("GARBAGE") > -1) {
					...
					for(var i = 1; i < package.length; i++) {
						...
						for(var j = 1; document.getElementById('materialNumbers_' + j); j++) {
							//found menu item
							if(document.getElementById('materialNumbers_' + j).value.search(package[i].split('x')[0]) > -1) {
								if(document.getElementById('deleted_' + j).checked == false) {
									...
									exists++;
									if(document.getElementById('materialNumbers_' + j).value == '3' + package[i].split('x')[0]) {
										rougeStatus = 1;
									}
									break;
								}
					...
					if(highLevel == '-1' && highLevelArray[0] == 'Y') {
						...
						for(var i = 1; i < highLevelArray.length; i++) {
							for(var j = 1; document.getElementById('materialNumbers_' + j); j++) {
								//found menu item
								if(document.getElementById('materialNumbers_' + j).value.search(highLevelArray[i]) > -1) {
									highLevel = document.getElementById('highLevelItemNumbers_' + j).value;
									if(document.getElementById('materialNumbers_' + j).value == '3' + highLevelArray[i]) {
										rougeStatus = 1;
									}
									break;
								}
	...
	console.log(package);
		...
		.type('input#numToAdd', '')
			...
			.evaluate(function(package) {
				var row = 0;
				var count = 0;
				for(count = 0; document.getElementById('materialNumbers_' + count); count++) {
				}
				count = count - 1;
				...
				for(var i = 3; i < (package.length - 1); i++) {
					...
					if(package[1] == 1) {
						document.getElementById('materialNumbers_' + count).value = '3' + package[i].split('x')[0];
					}
					else {
						document.getElementById('materialNumbers_' + count).value = package[i].split('x')[0];
					}
...
function updateValue(package) {
	console.log('>Update Value')
		nightmare
			...
			.evaluate(function(package) {
				/*
				for(var k = 1; document.getElementById('materialNumbers_' + k); k++) {
					if(document.getElementById('materialNumbers_' + k).value.search('79320') > -1) {
...
function addNew(package) {
	if(package[0]) {
		nightmare
			...
			.type('#materialNumbers_' + row, mealCode)
